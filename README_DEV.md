...
「演唱会足迹」是一个静态个人演唱会记录站点，用于沉淀作者观看过的所有演唱会回忆。每场演唱会都包含主题文案、现场图集、B站视频、完整歌单、座位信息、价格信息、城市标签等多维度信息，并通过时间轴、3D专辑堆叠、票根展示、动态文本等方式呈现。
...
```
ilive/
├── index.html              # 主页面入口（包含票根模态框、城市列表模态框、图片/视频/歌单查看器等）
...
│   │                       # 包含字段：artist, concertName, location, seat, price, date, poster, tags, description, images, video, videoUrl, songlist
...
│   ├── main.js             # 主逻辑（初始化、渲染、动画协调、3D专辑、票根展示、座位价格显示）
...
### 功能说明
点击场次统计卡片，会以精美票根形式展示所有演唱会信息。

### 票根设计
- **视觉效果**: 渐变色背景、锯齿边缘、海报图片展示
- **信息展示**: 艺人、演唱会名、日期、地点、座位、价格
- **交互效果**: 渐入动画、悬停效果
- **响应式**: 移动端自动调整布局
...
### 显示位置
1. **时间轴卡片**: 在地点信息下方显示座位和价格（同一行）
2. **票根卡片**: 在地点信息下方显示座位和价格（同一行）

### 视觉设计
- **座位图标**: 🪑 椅子图标
- **价格图标**: 🎫 票务图标
- **分隔符**: `|` 分隔
- **颜色**: 与整体色调统一，图标和文字均为白色（透明度 0.9）
